...
<form class="form"  action="add.php" method="post" autocomplete="off" enctype="multipart/form-data">
    ...
    <?php foreach ($projects as $project) : ?>
        ...
        <?php if ($project['id'] === get_post_value('project_id')) : ?>
            selected
        <?php endif; ?>>
            <?= $project['name']; ?>
        </option>
    ...
    <input class="form__input form__input--date <?= $classname; ?>"
        type="text" name="dt_deadline" id="date" value="<?= get_post_value('dt_deadline'); ?>"
        placeholder="Введите дату в формате ГГГГ-ММ-ДД">
    <?php if (!empty($errors['dt_deadline'])) : ?>
        <p class="form__message"><?= $errors['dt_deadline']; ?></p>
    <?php endif; ?>
    </div>
